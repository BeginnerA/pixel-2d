<template>
    ...
    <t-collapse v-model:value="activeCollapse" size="small" expand-icon-placement="right">
      <t-collapse-panel header="Tag标签">
        <t-space class="data-tag-block">
          <t-tag v-for="(tag, index) in editorPen.tags" :key="index" :closable="true" :max-width="100"
                 @close="closePenTag(index)">
            {{ tag }}
          </t-tag>
        </t-space>
        <t-space>
          <t-input v-model:value="inputPenTag" placeholder="按回车添加(最大长度100)" :maxlength="100" :clearable="true"
                   style="width: 238px" @blur="addPenTags" @enter="addPenTags"/>
        </t-space>
      </t-collapse-panel>
        ...
        <template v-if="editorPen.form !== undefined && editorPen.form.length !== 0">
          <t-form label-align="left" :label-width="70">
            <t-form-item v-for="(i, index) in editorPen.form as Array<Editor2DPropsMenu>" :label="i.title">
              <!-- 输入框 -->
              <t-input v-if="i.type && ['input','text'].includes(i.type)" :clearable="true" :disabled="i.disabled"
                       v-model="(i.data as any)[i.value]" style="width: 68%"
                       :placeholder="i.option?.placeholder || '请输入'" :type="i.option?.type||'text'"
                       @change="onUpdatePen(i.value, true)"/>
              <!-- 多行文本框 -->
              <t-textarea v-if="i.type==='textarea'" :disabled="i.disabled" v-model="(i.data as any)[i.value]"
                          :maxlength="i.option?.maxlength || 20" style="width: 68%"
                          :placeholder="i.option?.placeholder || '请输入'"
                          @change="onUpdatePen(i.value, true)"/>
              <!-- 数字框 -->
              <t-input-number v-else-if="i.type==='number'" :disabled="i.disabled" theme="column"
                              :placeholder="i.option?.placeholder || '请输入'" :step="i.option?.step || 1"
                              v-model="(i.data as any)[i.value]" :min="i.option?.min ?? -Infinity"
                              :max="i.option?.max ?? Infinity" @change="onUpdatePen(i.value, true)"/>
              <!-- 滑块 -->
              <t-slider v-else-if="i.type==='slider'" :disabled="i.disabled" style="width: 68%"
                        v-model="(i.data as any)[i.value]" :step="i.option?.step || 1" :min="i.option?.min ?? -Infinity"
                        :max="i.option?.max ?? Infinity" @change="onUpdatePen(i.value, true)"/>
              <!-- 选择框 -->
              <t-select v-else-if="i.type==='select'" :disabled="i.disabled" v-model="(i.data as any)[i.value]"
                        :placeholder="i.option?.placeholder" style="width: 68%"
                        @change="onUpdatePen(i.value, true)">
                <t-option
              ...
              <!-- 取色器 -->
              <t-color-picker v-else-if="i.type === 'color'" :disabled="i.disabled" v-model="(i.data as any)[i.value]"
                              :show-primary-color-preview="false" @change="onUpdatePen(i.value, true)"/>
              <!-- 开关 -->
              <t-switch v-else-if="i.type==='switch'" :disabled="i.disabled" v-model="(i.data as any)[i.value]"
                        @change="onUpdatePen(i.value, true)"/>
              <!-- 代码 -->
              <t-button v-else-if="i.type === 'code'" :disabled="i.disabled" :type="i.option?.type"
                        @change="onUpdatePen(i.value, true)" @click="openCodeEdit(i)">
                ...
              </t-button>
      ...
      <t-collapse-panel v-if="editorPen.dropdownList" header="下拉列表">
        <table class="set-select-table">
          ...
          <template v-for="(item, i) in dropdownList" :key="i">
            <tr>
              <td>
                <t-input v-model:value="item.text" placeholder="显示名称" size="small" style="width: 60px"
  ...
  <editor-form-item :options="{visible: editorFormVisible}" :model-value="editorFormData"
                    @confirm="confirmData" @close="openAddData"/>

  <editor-code :visible="editorCodeVisible" :model-value="editorCodeValue" :extend="editorCodeExtend"
...
function initFormData(pen: Editor2DPen) {
  ...
  if (!pen.form) {
    let form: any = {};
    if (['square', 'rectangle', 'circle', 'triangle', 'diamond', 'pentagon', 'hexagon', 'pentagram',
      'leftArrow', 'rightArrow', 'twowayArrow', 'cloud', 'message', 'file', 'cube', 'text'].includes(penName)) {
      form = {
        key: 'text',
        value: 'text',
        title: '文本',
