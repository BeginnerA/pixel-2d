  ...
  <div class="map-comm-config">
    <t-collapse expand-icon-placement="right" :default-value="activeCollapse">
      <t-collapse-panel header="WebSocket">
        <t-input v-model:value="editorMap.socketConfig['url']" :clearable="true" placeholder="请输入websocket地址"
      ...
      <t-collapse-panel header="MQTT">
        <t-form :data="editorMap.mqttConfig" :rules="mqttRules" label-align="left">
          ...
          <t-form-item label="Client ID">
            <t-input v-model:value="editorMap.mqttConfig['mqttOptions'].clientId" :clearable="true"
                     @blur="setMqttConfig"/>
          </t-form-item>
          <t-form-item label="自定义Client ID">
            <t-switch v-model:checked="editorMap.mqttConfig['mqttOptions'].customClientId" :clearable="true"
                      @change="setMqttConfig"/>
          </t-form-item>
          <t-form-item label="用户名">
            <t-input v-model:value="editorMap.mqttConfig['mqttOptions'].username" :clearable="true"
                     @blur="setMqttConfig"/>
          </t-form-item>
          <t-form-item label="密码">
            <t-input v-model:value="editorMap.mqttConfig['mqttOptions'].password" :clearable="true"
                     @blur="setMqttConfig"/>
          </t-form-item>
          <t-form-item label="Topics" name="mqttTopics">
            <t-input v-model:value="editorMap.mqttConfig['mqttTopics']" placeholder="多个topic以英文逗号“,”分隔"
                     @blur="setMqttConfig"/>
          </t-form-item>
        </t-form>
      </t-collapse-panel>
      <t-collapse-panel header="HTTP通信">
        <t-form label-align="left" label-width="70">
          <t-collapse-panel v-for="(item, index) in editorMap.httpConfig" :value="index"
                            :header="'事件 ' + (index + 1)">
    ...
    <editor-code :title="editorCodeTitle" :visible="editorCodeVisible" :model-value="editorCodeValue"
