    ...
    readFile(file: Blob): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            ...
            if (file.size > MAX_FILE_SIZE) {
                reject(new Error(`文件大小 ${(file.size / 1024 / 1024).toFixed(2)}MB 超过限制 ${MAX_FILE_SIZE / 1024 / 1024}MB`));
                return;
            }
            ...
            if (elem.files && elem.files[0]) {
                try {
                    ...
                    if (elem.files[0].type === 'image/svg+xml') {
                        ...
                        meta2d.canvas.addCaches = parseSvg(text);
                    } else {
                        // 文件是 JSON - 严格验证
...
export class SaveFile extends SaveEditor2DFile {
    ...
    savePngFile(): void {
        try {
            const name = meta2d.store.data.name || '未命名';
    ...
    saveSvgFile(): void {
        ...
        try {
            ...
            for (const pen of meta2d.store.data.pens) {
                if (pen.visible === false || !this.isShowChild(pen, meta2d.store)) {
                    continue;
                }
            ...
            a.setAttribute(
                "download",
                `${meta2d.store.data.name || '未命名'}.svg`
            );
