    ...
    <t-form :label-align="options.labelAlign" :layout="options.layout">
      <t-form-item v-for="i in formListData" :label="i.title">
        ...
        <t-textarea
          ...
          :disabled="i.disabled"
          :clearable="i.option?.clearable || true"
          :maxlength="i.option?.maxlength || 1024"
          @[i.event]="i.action"
          :placeholder="i.option?.placeholder || '请输入'"
        />
        <!-- 数字框 -->
        ...
        />
        <!-- 按钮 -->
        <t-button v-else-if="i.type === 'button'" :disabled="i.disabled" :type="i.option?.type" @[i.event]="i.action">
          {{ i.option?.title }}
        </t-button>
        <!-- 代码 -->
        <t-button
